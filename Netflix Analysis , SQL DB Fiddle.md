...
SELECT COUNT(title) AS "Movie Count"
FROM "netflix_titles_info"
WHERE title.type = 'Movie';
#How many movie titles are there in the database? (movies only, not tv shows), v2:
SELECT count(*)
FROM "netflix_titles_info"
WHERE type='Movie';
...
FROM "netflix_titles_info" title LEFT JOIN "netflix_people" people ON title.show_id =people.show_id
